...
class PointNavAgent(Agent):
    ...
    def _setup_actor_critic_agent(self, observation_spaces, action_space) -> None:
        ...
        pretrained_state = torch.load(
            self.config.RL.DDPPO.pretrained_weights, map_location=self.device
        )
    ...
    def _setup_vo_model(self) -> None:
        ...
        for k in model_names:
            self.vo_model[k] = vo_model_cls(
                ...
                # observation_strip=self.config.VO.REGRESS_MODEL.observation_strip,
                # observation_strip_proba=self.config.VO.REGRESS_MODEL.observation_strip_proba,
                observation_strip_type="None",
                hidden_size=self.config.VO.REGRESS_MODEL.hidden_size,
                backbone=self.config.VO.REGRESS_MODEL.visual_backbone,
                ...
                cls_action=self.config.VO.REGRESS_MODEL.cls_action,
                train_backbone=self.config.VO.REGRESS_MODEL.train_backbone,
                pretrain_backbone=self.config.VO.REGRESS_MODEL.pretrain_backbone,
                custom_model_path=self.config.VO.REGRESS_MODEL.custom_model_path,
            )
...
def main():
    ...
    config_paths = "/datasets/home/memmel/PointNav-VO/configs/point_nav_habitat_challenge_2020.yaml" # os.environ["CHALLENGE_CONFIG_FILE"]

    config = get_config(
        "/datasets/home/memmel/PointNav-VO/configs/rl/evaluation/debug.yaml", ["BASE_TASK_CONFIG_PATH", config_paths]
    ).clone()
